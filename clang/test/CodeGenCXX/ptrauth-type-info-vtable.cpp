...
// RUN:   %s -emit-llvm -o - | FileCheck %s --check-prefixes=CHECK,NODISC -DIKEY=0 -DDKEY=2
...
// RUN: %clang_cc1 -DENABLE_TID=0 -I%S -std=c++11 -triple=aarch64-none-elf \
// RUN:   -fptrauth-calls -fptrauth-intrinsics \
// RUN:   -fptrauth-vtable-pointer-type-discrimination \
// RUN:   -fptrauth-vtable-pointer-address-discrimination \
// RUN:   %s -emit-llvm -o - | FileCheck %s --check-prefixes=CHECK,NODISC -DIKEY=0 -DDKEY=2
...
// RUN:   -fptrauth-vtable-pointer-address-discrimination \
// RUN:   -fptrauth-noakey \
// RUN:   %s -emit-llvm -o - | FileCheck %s --check-prefixes=CHECK,NODISC -DIKEY=1 -DDKEY=3
...
// RUN:   %s -emit-llvm -o - | FileCheck %s --check-prefixes=CHECK,DISC -DIKEY=0 -DDKEY=2
...
// RUN: %clang_cc1 -DENABLE_TID=1 -I%S -std=c++11 -triple=aarch64-none-elf \
// RUN:   -fptrauth-calls -fptrauth-intrinsics \
// RUN:   -fptrauth-vtable-pointer-type-discrimination \
// RUN:   -fptrauth-vtable-pointer-address-discrimination \
// RUN:   -fptrauth-type-info-vtable-pointer-discrimination \
...
// RUN:   -fptrauth-type-info-vtable-pointer-discrimination \
// RUN:   -fptrauth-noakey \
// RUN:   %s -emit-llvm -o - | FileCheck %s --check-prefixes=CHECK,DISC -DIKEY=1 -DDKEY=3
...
// CHECK: @_ZTV10TestStruct = constant { [4 x ptr] } { [4 x ptr] [ptr null, ptr @_ZTI10TestStruct, ptr ptrauth (ptr @_ZN10TestStructD1Ev, i32 [[IKEY]], i64 52216, ptr getelementptr inbounds ({ [4 x ptr] }, ptr @_ZTV10TestStruct, i32 0, i32 0, i32 2)), ptr ptrauth (ptr @_ZN10TestStructD0Ev, i32 [[IKEY]], i64 39671, ptr getelementptr inbounds ({ [4 x ptr] }, ptr @_ZTV10TestStruct, i32 0, i32 0, i32 3))] }, align 8

// NODISC: @_ZTI10TestStruct = constant { ptr, ptr } { ptr ptrauth (ptr getelementptr inbounds (ptr, ptr @_ZTVN10__cxxabiv117__class_type_infoE, i64 2), i32 [[DKEY]]), ptr @_ZTS10TestStruct }, align 8

// DISC: @_ZTI10TestStruct = constant { ptr, ptr } { ptr ptrauth (ptr getelementptr inbounds (ptr, ptr @_ZTVN10__cxxabiv117__class_type_infoE, i64 2), i32 [[DKEY]], i64 [[STDTYPEINFO_DISC]], ptr @_ZTI10TestStruct), ptr @_ZTS10TestStruct }, align 8
...
// NODISC: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[CAST_VPTR]], i32 [[DKEY]], i64 0)
...
// DISC: [[AUTHED:%.*]] = call i64 @llvm.ptrauth.auth(i64 [[VPTRI]], i32 [[DKEY]], i64 [[DISCRIMINATOR]])
