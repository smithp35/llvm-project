...
extern "C" {
HIDDEN_ATTR extern uintptr_t __rela_dyn_start;
HIDDEN_ATTR extern uintptr_t __rela_dyn_end;
}
...
HIDDEN_ATTR void init_global_pointers(void) {
  ...
  for (Relocation *rp = rstart; rp != rend; ++rp) {
    ...
    case R_AARCH64_AUTH_RELATIVE: {
      ...
      if (key == APIA || key == APDA) {
        exceptions::print_hex(rp->r_offset);
        exceptions::print_str(" Signing with A key,but A key disabled\n");
      }
#elif __has_feature(ptrauth_nobkey)
      if (key == APIB || key == APDB) {
        exceptions::print_hex(rp->r_offset);
        exceptions::print_str(" Signing with B key,but B key disabled\n");
      }
#endif
